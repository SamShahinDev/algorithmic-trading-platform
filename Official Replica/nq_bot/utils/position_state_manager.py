...
class PositionStateManager:
    ...
    async def sync_with_broker(self, force: bool = False) -> bool:
        """
        Sync with broker using dynamic intervals with retry logic
        """
        max_retries = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(f"Syncing positions: {reason} (attempt {attempt + 1}/{max_retries})")
                
                # Validate token before making request
                if hasattr(self.broker, 'validate_token'):
                    if not await self.broker.validate_token():
                        logger.info("POS_SYNC token_invalid, refreshing")
                        if await self.broker.refresh_token():
                ...
                if not response or not response.get('success'):
                    ...
                    if 'disconnected' in str(error_msg).lower() or 'unauthorized' in str(error_msg).lower():
                        logger.warning(f"POS_SYNC disconnected, attempt {attempt + 1}/{max_retries}")
                        
                        # Try to refresh token
                        if hasattr(self.broker, 'refresh_token'):
                            if await self.broker.refresh_token():
                                logger.info("TOKEN_REFRESH ok, retrying sync")
                    ...
                    logger.warning(f"Position sync failed: {error_msg}")
                ...
                # Success - process positions
                broker_positions = response.get('positions', [])
                
                # Log success
                logger.info(f"POS_SYNC ok positions={len(broker_positions)}")
                ...
                if 'server disconnected' in error_str or 'connection' in error_str:
                    logger.warning(f"POS_SYNC connection_error attempt={attempt + 1}: {e}")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(retry_delay * (attempt + 1))
                        continue
                else:
        ...
        logger.error(f"POS_SYNC failed after {max_retries} attempts")
        return False
