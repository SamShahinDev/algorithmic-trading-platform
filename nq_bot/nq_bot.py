...
# Ensure proper import paths - add current directory first for utils
...
# Import utils modules at module level
...
class NQBotWithPatterns:
    ...
    def __init__(self):
        ...
        from nq_bot.pattern_integration import PatternManager
    ...
    async def execute_trade(self, signal_data):
        ...
        try:
            # Check if already in position (check both position sources)
    ...
    async def start(self):
        ...
        try:
            from nq_bot.pattern_config import (DISCOVERY_MODE, LIVE_MARKET_DATA, DISABLE_REGIME_GATING, 
                ...
                if has_data and self.current_price:
                    ...
                    if self.position or self.execution_manager.current_position:
                        ...
                        if not last_signal_time or (current_time - last_signal_time).seconds > signal_cooldown:
                            ...
                            if signal:
                                # Write IMMEDIATE_EXEC telemetry
                                try:
                                    from nq_bot.utils.telemetry_sink import get_telemetry_sink
                                    sink = get_telemetry_sink()
                                    if sink:
                                        pattern_name = signal.get('pattern_name', 'unknown')
                                        signal_obj = signal.get('signal')
                                        confidence = getattr(signal_obj, 'confidence', 0) if signal_obj else 0
                                        sink.write_telemetry('IMMEDIATE_EXEC', {
                                            'pattern': pattern_name,
                                            'confidence': confidence,
                                            'timestamp': current_time.isoformat()
                                        })
                                except Exception as e:
                                    logger.debug(f"IMMEDIATE_EXEC telemetry failed: {e}")
...
async def main():
    ...
    # Check strategy mode for display
    from pattern_config import STRATEGY_MODE
    
    if STRATEGY_MODE == "FVG_ONLY":
        print("\n" + "=" * 60)
        print(" NQ TRADING BOT - FVG ONLY MODE")
        print(" Fair Value Gap Strategy")
        print("=" * 60)
