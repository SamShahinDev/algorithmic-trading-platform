...
class DataCache:
    ...
    async def warmup(self, lookback_1m: int = 200) -> bool:
        ...
        try:
            # Use hardcoded values to avoid import issues
            ATR_BASELINE_LOOKBACK_BARS = 200
            ATR_BASELINE_MIN_FALLBACK = 10.0
            ...
            if not df.empty:
                ...
                self.data_1m = self.bars_1m  # CRITICAL: Assign to data_1m for get_bars() to work!
                self._assert_utc_index()
                self._rebuild_higher_tfs()
    ...
    async def update_incremental(self) -> bool:
        ...
        try:
            ...
            # CRITICAL: Sync bars_1m with data_1m for compatibility
            self.bars_1m = self.data_1m
    ...
    def get_bars(self, timeframe: Literal['1m', '5m', '1h'], limit: Optional[int] = None) -> pd.DataFrame:
        ...
        Args:
            ...
            limit: Optional number of bars to return (returns last N bars)
        ...
        data = mapping.get(timeframe, pd.DataFrame())
