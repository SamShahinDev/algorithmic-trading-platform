...
class TopStepXClient:
    ...
    async def _get_contract_id(self, symbol: str = "NQ") -> Optional[str]:
        ...
        contract_map = {
            "NQ": "CON.F.US.ENQ.Z25",  # E-mini NASDAQ-100 (NQZ5)
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if response.status != 0:  # Only log as error if not status 0
    ...
    async def retrieve_bars(
        ...
        *,
        ...
        try:
            ...
            # Debug logging
            logger.info(f"retrieve_bars request: contract={contract_id}, live={live}, start={start_time.isoformat()}, end={end_time.isoformat()}")
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if not bars:
                        logger.warning(f"retrieve_bars returned empty bars for {contract_id}, response: {res}")
